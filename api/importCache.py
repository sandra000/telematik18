...
class MainImport(object):
    ...
    def get_parameter(self, period_id, time_start, time_end, limit):
        result = self.session.query(models.Parameter).filter(models.Parameter.period_id == period_id).filter(
            models.Parameter.time_start == time_start).filter(models.Parameter.time_end == time_end).filter(
            models.Parameter.limit == limit).first()
        if result:
            return result
        return -1
    ...
    def update_ohcl_histories(self, symbol, period='1DAY', start_time=date(2018, 1, 1).isoformat(),
                              end_time=date.today().isoformat(), limit=10000):
        ...
        if parameter == -1:
            ...
            self.session.execute(
                'DELETE FROM histories WHERE symbol_id=' + str(symbol_id) + ' AND parameter_id=' + str(parameter.id))
            self.session.commit()
